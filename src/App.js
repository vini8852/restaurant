...
class App extends Component {
  ...
  async componentDidMount() {
    const response = await axios.get(
      "https://" + process.env.REACT_APP_backend + "/restaurant"
    );
    ...
    const response1 = await axios.get(
      "https://" + process.env.REACT_APP_backend + "/restaurant/cuisines"
    );
    ...
    this.setState({
      ...this.state,
      data: data_.Restrants,
      Count: data_.Count,
      hasNxt,
      cuisines: [...this.state.cuisines, ...cuisines],
  ...
  render() {
    ...
    const handlePageDec = async () => {
      if (this.state.page > 1) {
        ...
        var elementsCount = document.getElementById("Restrantresult")
          .childElementCount;

        this.setState({ skip: elementsCount * this.state.page }, () =>
          handleSearch()
        );
      }
    };
    ...
    const handleSearch = async () => {
      ...
      const r = await axios.post(
        "https://" + process.env.REACT_APP_backend + "/restaurant/search",
        {
          obj,
        }
      );
        ...
        <section className={"container"}>
          <Row>
            <Col md={{ span: 10 }}>
              <input
                className={"g inp"}
                value={this.state.search_rest}
                onChange={(e) => handleRest(e)}
                placeholder={"Restrant   "}
              />{" "}
            </Col>
            <Col md={{ span: 10 }}>
              <AutoComplete
            ...
            <Col md={{ span: 4 }}>
              <button className={"write"} onClick={handleSrch}>
                Search
              </button>
            </Col>
          </Row>
          ...
          <Row>
            <Col
              md={{ span: 6, offset: 1 }}
              style={{ borderRight: "2px solid #14b38e" }}
            >
              {" "}
              <b>Rating </b>
              <Icon
            ...
            <Col md={{ span: 8, offset: 1 }}>
              ...
              <Icon
                onClick={() => {
                  handleSort("Average Cost for two", -1);
                }}
                style={{ cursor: "pointer" }}
                type="arrow-down"
              />
          ...
          <Row>
            ...
            <Col md={{ span: 8, offset: 4 }}>
              {" "}
              <span>
                About {this.state.Count} results <Divider type="vertical" />{" "}
            ...
            <Col md={{ span: 2, offset: 6 }}>
              {" "}
              <Button
                disabled={this.state.hasNxt ? "" : "disabled"}
                type="dashed"
